...
def load_data():
    ...
    audit_log = pd.read_csv(
        "data/audit/dq_corrections.csv", parse_dates=["transformed_at"]
    )
...
tab_dq, tab_business, tab_audit = st.tabs(
    ["Data Quality", "Business Metrics", "Audit Trail"]
)
...
with tab_audit:
    ...
    if search:
        filtered = audit_log[
            audit_log["policy_id"].str.contains(search, case=False, na=False)
        ]
    else:
        filtered = audit_log
